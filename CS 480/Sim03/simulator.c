...
void runSim ( ConfigDataType *configPtr, OpCodeType *metaDataMstrPtr )
{
  ...
  int averageCycleTime = ( configPtr->procCycleRate +
                                                configPtr->quantumCycles ) / 2;
  ...
  char cpuSchedCodeString[ HIGHEST_PID ];
  ...
  else if ( compareString( cpuSchedCodeString, "SJF-N" ) == STR_EQ )
  {
    ...
    currentPCB = shortestJob( headPCB, currentPCB );
  ...
  if( displayToMonitorFlag == True )
   {
     ...
     displaySimToMonitor( currentPCB, currentPCB -> metaDataPtr, headPCB,
                              displayToMonitorFlag, configPtr -> memAvailable,
                    ...
                    Boolean displayFlag, int memFromConfig, int procCycleRate )
{
  ...
  char timeStr[ STD_STR_LEN ];
  int memForPCB = memFromConfig, calculatedMem, calculatedThread;
  ...
  while( currentPCB -> processState == READY_STATE )
    {
      ...
      while( compareString(currentCmd->strArg1, "end") != STR_EQ )
        {
          calculatedThread = currentCmd -> intArg2 * procCycleRate;
        ...
        if( compareString( currentCmd->command, "dev" ) == STR_EQ )
        {
        // checks each of the string arguments
        if ( compareString( currentCmd->inOutArg, "in" ) == STR_EQ )
          {
          // check for hard drive, sound signal, video signal, keyboard, memory
        ...
        if( compareString( currentCmd->strArg1, "mem allocate" ) == STR_EQ )
           {
             accessTimer(LAP_TIMER, timeStr);
             printf("%s, Process %d, memory allocation request\n",
                                                     timeStr, currentPCB->pid);

             calculatedMem = currentCmd -> intArg3 + currentCmd -> intArg2;

             accessTimer(LAP_TIMER, timeStr);
             if ( memForPCB - calculatedMem > 0 )
             {
               currentPCB = ( PCB * ) malloc( sizeof( calculatedMem ) );
